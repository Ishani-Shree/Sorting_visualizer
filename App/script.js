...
function generateBars(n = 100) {
    bars = [];
    let container = document.getElementById("container");

	let barWidth = 10;
    // Ensure bars array and container content are updated
    for (let i = 0; i < n; i++) {
        bars.push('<div class="bar" id="' + i + '" style="height:' + Math.floor(2 + Math.random() * 98) + '%; width: ' + barWidth + 'px;"></div>');
    }
...
window.addEventListener('resize', () => {
    generateBars(parseInt(document.getElementById("bars").value));
});
//=============================== Sorting Algorithms ==================================//
